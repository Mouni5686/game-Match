...
class App extends Component {
  ...
  statusChange = () => {
    ...
    if (timer !== 0) {
      this.setState(prevtime => ({timer: prevtime.timer - 1}))
    } else {
  ...
  render() {
    const {score, timer, category, imgUrl, isTrue} = this.state
          ...
          {!isTrue && (
            <div className="first-div">
                  ...
                  <li key={eachTab.tabId}>
                    <button
                      ...
                      className={`tab-button ${
                        category === eachTab.tabId ? 'highlight-text' : ''
                      }`}
              ...
              <ul className="thumbnails-list">
                {thumbnailList.map(eachThumbnail => (
                  <li key={eachThumbnail.id}>
                    <button
                      className="button"
                      type="button"
                      onClick={() =>
                        this.onClickThumb(eachThumbnail.thumbnailUrl)
                      }
                    >
                      <img
